...
mod tests {
    ...
    fn aaa_floor_one_preparation_and_graph_match_official_oracle() {
        ...
        let mut level = Level::new(1, prepared.feeling);
        normalize_rooms(&mut level, &mut graph.rooms);
        assert_eq!((level.width(), level.height()), (37, 43));
        ...
        assert_eq!(
            signatures,
            [
                "Connection(RingBridge)@22,15,27,23",
                "Connection(RingTunnel)@27,11,33,17",
                "Connection(Tunnel)@1,27,10,34",
                "Connection(Tunnel)@21,1,27,6",
                "Connection(Tunnel)@5,13,11,21",
                "Entrance(RegionDecoPatch)@14,4,21,11",
                "Exit(WaterBridge)@10,23,19,30",
                "Special(Pool)@22,8,27,15",
                "Special(Runestone)@20,23,25,28",
                "Standard(RegionDecoPatch)@1,21,8,27",
                "Standard(SewerPipe)@11,11,22,23",
                "Standard(SewerPipe)@2,34,10,41",
                "Standard(SewerPipe)@27,3,35,11",
            ]
        );
        assert_eq!(
    ...
    fn additional_floor_one_graphs_match_official_oracle() {
        let fixtures: [(&str, (i32, i32), &[&str]); 3] = [
            (
                ...
                (29, 33),
                &[
                    "Connection(Tunnel)@10,24,15,31",
                    "Connection(Tunnel)@12,15,15,20",
                    "Connection(Tunnel)@13,1,18,6",
                    "Connection(Tunnel)@15,13,19,21",
                    "Entrance(RegionDecoPatch)@7,2,13,11",
                    "Exit(WaterBridge)@19,8,27,15",
                    "Special(CrystalPath)@19,15,25,21",
                    "Special(Runestone)@3,24,10,31",
                    "Standard(Ring)@1,11,12,24",
                    "Standard(Ring)@15,21,23,28",
                    "Standard(WaterBridge)@18,1,25,8",
                ],
            ),
            (
                "ABC-DEF-GHI",
                (43, 29),
                &[
                    "Connection(Tunnel)@10,4,18,7",
                    "Connection(Tunnel)@12,8,18,13",
                    "Connection(Tunnel)@18,23,25,27",
                    "Connection(Tunnel)@8,18,10,23",
                    "Connection(Walkway)@10,19,13,24",
                    "Entrance(WaterBridge)@25,14,33,20",
                    "Exit(RegionDecoPatch)@4,9,12,18",
                    "Special(MagicalFire)@33,14,41,22",
                    "Standard(RegionDecoPatch)@25,20,33,27",
                    "Standard(SewerPipe)@1,1,10,9",
                    "Standard(SewerPipe)@13,13,25,23",
                    "Standard(WaterBridge)@18,5,27,13",
                ],
            ),
            (
                "ZZZ-ZZZ-ZZZ",
                (33, 35),
                &[
                    "Connection(Tunnel)@12,26,19,33",
                    "Connection(Tunnel)@17,3,21,7",
                    "Connection(Tunnel)@19,9,23,13",
                    "Connection(Tunnel)@21,1,23,8",
                    "Connection(Tunnel)@26,12,31,18",
                    "Connection(Tunnel)@6,19,12,24",
                    "Connection(Tunnel)@7,5,14,12",
                    "Entrance(RegionDecoPatch)@12,12,19,21",
                    "Exit(WaterBridge)@23,5,31,12",
                    "Special(Armory)@1,5,7,13",
                    "Special(Storage)@19,13,23,18",
                    "Standard(CircleBasin)@19,18,31,30",
                    "Standard(Platform)@5,24,12,31",
                    "Standard(SewerPipe)@5,13,12,19",
                    "Standard(WaterBridge)@14,7,19,12",
                ],
            ),
        ];
